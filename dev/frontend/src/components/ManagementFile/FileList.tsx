...
import { showConfirmationAlert, showSuccessAlert, showErrorAlert } from '../../utils/alertUtils';
import { deleteCsvFile, deleteCsvFilePermanently, restoreCsvFile, CsvDataType } from '../../databaseUtils/Csvs';
...
const FileList: React.FC<FileListProps> = ({ activeCsvList, deleteCsvList, loadingCsvList, fetchCsvList }) => {
    ...
    const handleRestore = async (csvId: string, fileName: string, fetchCsvList: () => Promise<void>) => {
        showConfirmationAlert(
            '復元確認',
        ...
        <Card sx={{ p: 1, m: 5, borderRadius: '25px' }}>
            <CardContent>
                ...
                {loadingCsvList ? (
                    ...
                    <TableContainer component={Paper} sx={{ maxHeight: 800, borderRadius: '25px' }}>
                        <Table>
                            <TableHead>
                                <TableRow sx={{ backgroundColor: '#F4F4F4' }}>
                                    {isTrashMode && (
                                        <TableCell align="center" sx={{ fontWeight: 'bold', fontSize: '18px' }}></TableCell>
                                    )}
                            ...
                            <TableBody>
                                {displayList.length > 0 ? (
                                        ...
                                        <TableRow key={index}>
                                            {isTrashMode && (
                                                <TableCell align="center">
                                                    <IconButton
                                                        onClick={() => handleRestore(csv.csv_id, csv.file_name.replace(/\.csv$/, ''), fetchCsvList)}
                                                    >
                                            ...
                                            <TableCell align="center">
                                                {isTrashMode ? (
                                                    ...
                                                    <Button
                                                        variant={selectedCsvId === csv.csv_id ? "contained" : "outlined"}
                                                        onClick={() => handleSelect(csv.csv_id)}
                                                        sx={{ borderRadius: '10px', fontWeight: 'bold', minWidth: '200px' }}
                                                    >
                                                        {csv.file_name.replace(/\.csv$/, '')}
                                                    </Button>
                                                )}
                                    ...
                                    <TableRow>
                                        <TableCell colSpan={6} align="center" sx={{ fontSize: '20px' }}>
                                            {isTrashMode
                                                ? 'ゴミ箱は空です。'
